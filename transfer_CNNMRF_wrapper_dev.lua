...
local function main(params)
  ...
  -- read images
  -----------------------------------------------------------------------------------
  local source_image = image.load(string.format('%s', params.content_name), 3)
  ...
  -- --------------------------------------------------------------------------------------------------------
  -- -- local function for adding a mrf layer, with image rotation and scaling
  -- --------------------------------------------------------------------------------------------------------
  ...
  local function maybe_save(t)
    ...
    local filename = string.format('%s/res_%d_%d.png', params.output_folder, cur_res, t)
...
--    }
	...
	--local noise_image = image.load(string.format('%s', params.noise_name), 3)
	--noise_image = image.scale(noise_image, params.max_size, 'bilinear')
	--local noise_image_caffe = preprocess(noise_image):float()
	--noise_image_caffe = image.scale(noise_image_caffe, math.ceil(noise_image:size()[3] * math.pow(0.5, params.num_res - i_res)), math.ceil(noise_image:size()[2] * math.pow(0.5, ------params.num_res - i_res)), 'bilinear')
	--input_image = noise_image_caffe:clone():float()
	  ...
	  if is_pooling and params.pooling == 'avg' then
	    ...
	    local kkW, kkH = layer.kW, layer.kH
	    local ddW, ddH = layer.dW, layer.dH
	    local avg_pool_layer = nn.SpatialAveragePooling(kkW, kkH, ddW, ddH):float()
	    if params.gpu >= 0 then
	      if params.backend ~= 'clnn' then
		avg_pool_layer:cuda()
	      else
...
--      local x, losses = optim.lbfgs(feval, input_image, optim_state)
    ...
    elseif params.optimizer == 'adam' then
      ...
      for t = 1, params.num_iter[i_res] do
